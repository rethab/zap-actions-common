...
export function getRunnerID(body: string) {
...
export async function processLineByLine(tsvFile: string) {
    let plugins = [];
    try {
...
export function createMessage(sites: [], runnerID: unknown, runnerLink: unknown) {
    const NXT_LINE = '\n';
    const TAB = "\t";
    const BULLET = "-";
    let msg = '';
...
function generateDifference(newReport: any, oldReport) {
...
async function readMDFile(reportName: string) {
    let res = '';
    try {
...
function checkIfAlertsExists(jsonReport: string) {
    return jsonReport.site.some((s) => {
        return (s.hasOwnProperty('alerts') && s.alerts.length !== 0);
    });
}

async function filterReport(jsonReport: string, plugins: []) {
...
export function readPreviousReport(octokit: string, owner: string, repo: string, workSpace: string, runnerID: number) {
    let previousReport;
    try{
...
export async function uploadArtifacts(rootDir: string, mdReport: string, jsonReport: string, htmlReport: string, artifactName = 'zap_scan') {
